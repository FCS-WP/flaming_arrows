...
if ( ! function_exists( 'putter_add_css_vars' ) ) {
	...
	function putter_add_css_vars( $css, $args ) {
						...
						if ( is_array( $font ) ) {
							foreach ( $font as $css_prop => $css_value ) {
								...
								if ( empty( $suffix ) || ! empty( $font["{$css_prop}{$suffix}"] ) ) {
									...
									if ( ( ! empty( $font["{$param}{$suffix}"] ) && ! putter_is_inherit( $font["{$param}{$suffix}"] ) ) || strpos( $css_prop, 'color' ) === false ) {
										$tmp .= "--theme-font-{$tag}_{$css_name}: " . ( ! empty( $font["{$param}{$suffix}"] ) && ! putter_is_inherit( $font["{$param}{$suffix}"] )
																		? ( in_array( $css_prop, array( 'font-size', 'letter-spacing', 'margin-top', 'margin-bottom', 'border-width', 'border-radius' ) )
																			? putter_prepare_css_value( $font["{$param}{$suffix}"] )
																			: $font["{$param}{$suffix}"]
																			)
																		: 'inherit'
																	) . ";\n";
		...
		if ( isset( $css['vars'] ) && isset( $args['vars'] ) ) {
			...
			if ( is_array( $PUTTER_STORAGE['options'] ) && count( $PUTTER_STORAGE['options'] ) > 0 ) {
				$breakpoints = putter_get_theme_breakpoints();
				foreach ( $breakpoints as $bp => $bpv ) {
				...
				foreach ( $PUTTER_STORAGE['options'] as $option => $value ) {
					if ( ! empty( $value['css'] ) ) {
						...
						foreach ( $breakpoints as $bp => $bpv ) {
							...
							if ( $css_value !== '' ) {
								$css_value = putter_prepare_css_value( $css_value );
