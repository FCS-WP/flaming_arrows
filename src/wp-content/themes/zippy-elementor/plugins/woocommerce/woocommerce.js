...
jQuery( document ).on( 'action.ready_putter', function() {
	...
	$document.on( 'action.resize_putter', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.putter_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		putter_set_cookie( 'putter_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	putter_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function putter_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						putter_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function putter_woocomerce_inc_dec_click(button) {
	...
	$document.on( 'action.scroll_putter', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < putter_window_scroll_top() + putter_window_height() * 1.5 ) {
	...
	//-------------------------------------------------------------------------
	if ( ! PUTTER_STORAGE['select_container_disabled'] ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		putter_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					putter_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					putter_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		putter_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			putter_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function putter_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = putter_window_width() + ( putter_window_height() < $document.height() || putter_window_scroll_top() > 0 ? 16 : 0 );
				}
				...
				if ( w >= PUTTER_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
