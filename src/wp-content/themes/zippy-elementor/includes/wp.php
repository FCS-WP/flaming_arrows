...
if ( ! function_exists( 'putter_redirect_404' ) ) {
	add_action( 'template_redirect', 'putter_redirect_404', 1 );
	/**
	 * Redirect to the 404 page if the page is selected in the theme options.
	 * 
	 * @hooked template_redirect
	 */
	function putter_redirect_404() {
		if ( is_404() ) {
			$page_id = putter_get_theme_option( 'redirect_404_page', '' );
			if ( ! putter_is_off( $page_id ) ) {
				$way = apply_filters( 'putter_filter_redirect_404_way', 'show' );	// show | redirect
				if ( $way == 'redirect' ) {
...
if ( ! function_exists( 'putter_create_widgets_area' ) ) {
	...
	function putter_create_widgets_area( $name, $add_classes = '' ) {
		$widgets_name = putter_get_theme_option( $name, 'hide' );
		...
		if ( putter_sidebar_present() ) {
			...
			if ( 'default' == $putter_sidebar_type ) {
				...
				if ( is_active_sidebar( $putter_sidebar_name ) ) {
					$sb_width = (int)putter_get_theme_option( 'sidebar_width' );
					$sb_gap   = (int)putter_get_theme_option( 'sidebar_gap' );
				}
			} else {
				$sb_width = (int)putter_get_theme_option( 'sidebar_width' );
				$sb_gap   = (int)putter_get_theme_option( 'sidebar_gap' );
...
if ( ! function_exists( 'putter_get_date' ) ) {
	 ...
	 */
	function putter_get_date( $dt = '', $format = '', $disable_date_diff = false ) {
		$diff   = $disable_date_diff ? 0 : intval( putter_get_theme_option( 'time_diff_before' ) );
		...
		if ( $diff == 0 || $dt_cur - $dt > $diff * 24 * 3600 ) {
			...
			$dt = sprintf( esc_html__( '%s ago', 'putter' ), human_time_diff( $dt, $dt_cur ) );
		}
		return $dt;
	}
}
...
if ( ! function_exists( 'putter_get_upgrade_url' ) ) {
	...
	function putter_get_upgrade_url( $params = array() ) {
		...
		if ( ! empty( $params['action'] ) && ! in_array( $params['action'], array( 'info_skins' ) ) ) {
			...
			$default['domain']     = putter_remove_protocol_from_url( home_url(), true );
