	...
	function() {
		...
		jQuery( '.putter_scheme_editor_control_reset' )
			.on( 'click', function() {
				if (confirm( PUTTER_STORAGE['msg_scheme_reset'] )) {
					...
					scheme                               = selector.val();
					putter_color_schemes[scheme].colors = putter_clone_object( typeof putter_color_schemes_backup[scheme].colors_factory != 'undefined' ? putter_color_schemes_backup[scheme].colors_factory : putter_color_schemes_backup[scheme].colors );
			...
			.on( 'click', function() {
				...
				if (title) {
					...
					putter_color_schemes_backup[scheme_new] = {
						...
						'colors_factory': putter_clone_object( typeof putter_color_schemes[scheme].colors_factory != 'undefined' ? putter_color_schemes[scheme].colors_factory : putter_color_schemes[scheme].colors )
					};
		...
		function putter_init_fields(e, container) {
			...
			if (container.find( '.putter_color_selector' ).length > 0) {
				...
				container.find( '.putter_color_selector' ).each( function() {
					var fld = jQuery( this );
					...
					if ( globals.length ) {
						fld.on( 'change', function() {
							...
							globals.find( '.putter_color_selector_globals_list_item_active' ).removeClass( 'putter_color_selector_globals_list_item_active' );
							globals.removeClass( 'putter_color_selector_globals_active' );
						} );
					}
					...
					if ( globals.length && globals.hasClass( 'putter_color_selector_globals' ) ) {
						globals
							// Open/close global colors list on click on the button
							.on( 'click', '.putter_color_selector_globals_button', function() {
								globals.toggleClass( 'putter_color_selector_globals_list_opened' );
							} )
							// Select color from the list
							.on( 'click', '.putter_color_selector_globals_list_item', function() {
								globals.find( '.putter_color_selector_globals_list_item_active' ).removeClass( 'putter_color_selector_globals_list_item_active' );
								var $self = jQuery( this ).addClass( 'putter_color_selector_globals_list_item_active' ),
									$fld_globals = globals.find( 'input[type="hidden"]' ),
									color = $self.data( 'color' ),
									value = $self.data( 'value' );
								// Set a new value to the colorpicker field
								if ( color !== undefined ) {
									globals.removeClass( 'putter_color_selector_globals_list_opened' );
								...
								if ( value !== undefined ) {
									setTimeout( function() {
										...
										$self.addClass( 'putter_color_selector_globals_list_item_active' );
										globals.addClass( 'putter_color_selector_globals_active' );
									}, 10 );
								}
