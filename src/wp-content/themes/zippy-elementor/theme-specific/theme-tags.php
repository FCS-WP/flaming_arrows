	...
	function putter_show_post_title_and_meta( $args = array() ) {
		...
		if ( ( ! putter_sc_layouts_showed( 'title' ) || ! putter_sc_layouts_showed( 'postmeta' ) ) ) {
			...
			<div class="post_header post_header_single entry-header">
				...
				$share_position  = putter_array_get_keys_by_value( putter_get_theme_option( 'share_position', 'top=0|left=0|bottom=1' ) );
	...
	function putter_show_post_footer( $components = 'pages,tags,likes,share,prev_next,author' ) {
		...
		foreach( $components as $comp ) {
			...
			if ( 'tags' == $comp ) {
				...
				// Emotions
				if ( putter_exists_trx_addons() && function_exists( 'trx_addons_get_post_reactions' ) && trx_addons_is_on( trx_addons_get_option( 'emotions_allowed', 0 ) ) ) {
				...
				if ( in_array( 'likes', $components ) && in_array( 'likes', $meta_components )
						&&
						( ! function_exists( 'trx_addons_get_option' ) || trx_addons_is_off( trx_addons_get_option( 'emotions_allowed', 0 ) ) || ! apply_filters( 'trx_addons_filter_show_post_reactions', putter_is_single() && ! is_attachment() ) )
				) {
					$meta_footer[] = 'likes';
				}
				...
				if ( putter_get_theme_option( 'show_author_info' ) == 1
					&& ! is_attachment()
					&& ( apply_filters( 'putter_filter_show_author_without_bio', true ) || get_the_author_meta( 'description' ) )
					&& ( 'scroll' != $putter_posts_navigation || (int)putter_get_theme_option( 'posts_navigation_scroll_hide_author', 0 ) == 0 )
					&& ( ! $full_post_loading || (int)putter_get_theme_option( 'open_full_post_hide_author', 1 ) == 0 )
				) {
				...
				if ( 'links' == $putter_posts_navigation && ! $full_post_loading ) {
					...
					<div class="nav-links-single<?php
						...
						if ( ! putter_is_off( putter_get_theme_option( 'posts_navigation_fixed', 0 ) ) ) {
	...
	function putter_show_post_meta( $args = array() ) {
		...
		foreach ( $components as $comp ) {
			...
			if ( 'categories' == $comp ) {
				...
				// Published date
				$dt = apply_filters( 'putter_filter_get_post_date', putter_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '', ! empty( $args['disable_date_diff'] ) ) );
				...
				// Modified date
				$dt = apply_filters( 'putter_filter_get_post_modified_date', putter_get_date( get_post_modified_time( 'U' ), ! empty( $args['date_format'] ) ? $args['date_format'] : '', ! empty( $args['disable_date_diff'] ) ) );
				if ( ! empty( $dt ) && ( empty( $dt_last ) || $dt_last != $dt ) ) {
					putter_show_layout(
						$dt,
...
if ( ! function_exists( 'putter_show_post_featured' ) ) {
	...
	function putter_show_post_featured( $args = array() ) {
		$args = apply_filters( 'putter_filter_post_featured_args', array_merge(
			array(
				'popup'         => ! putter_is_single() && (int)putter_get_theme_option( 'video_in_popup', 0 ) > 0,  // Open video in popup
				'hover'         => putter_get_theme_option( 'image_hover' ),     // Hover effect
				'parallax'      => (int)putter_get_theme_option( 'single_parallax', 0 ), // Parallax speed. If 0 - no parallax effect is used
		...
		if ( ! empty( $args['singular'] ) ) {
			...
			if ( is_attachment() ) {
				...
				if ( ( ( $has_thumb || ! empty( $args['show_no_image'] ) ) && ( ! putter_sc_layouts_showed( 'featured' ) || ! empty( $args['force'] ) ) )
		...
		if ( ! $args['thumb_only'] && ( in_array( $args['post_format'], array( 'video', 'audio', 'gallery' ) ) || ! empty( $args['video'] ) ) ) {
			...
			if ( 'video' == $args['post_format'] || ! empty( $args['video'] ) ) {
				if ( ! empty( $args['singular'] ) 
					...
					} else if ( putter_get_theme_option( 'blog_content', 'excerpt' ) != 'fullpost' ) {
				...
				if ( empty( $args['singular'] ) ) {
					if ( empty( $audio ) && putter_get_theme_option( 'blog_content', 'excerpt' ) != 'fullpost' ) {
...
if ( ! function_exists( 'putter_get_no_image' ) ) {
	 ...
	 */
	function putter_get_no_image( $no_image = '', $need = false, $return_url = true ) {
		...
		if ( empty( $img ) && ( $need || putter_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( $return_url ) {
				...
				if ( empty( $no_image_dir ) ) {
					$no_image_dir = putter_get_file_dir( 'images/no-image.jpg' );
				}
				$img = $no_image_dir;
			}
		}
...
if ( ! function_exists( 'putter_show_related_posts_callback' ) ) {
	...
	function putter_show_related_posts_callback() {
		if ( putter_is_single() && ! apply_filters( 'putter_filter_show_related_posts', false ) ) {
			...
			$putter_related_style   = putter_get_theme_option( 'related_style', 'classic' );
...
if ( ! function_exists( 'putter_show_related_posts' ) ) {
	...
	function putter_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int)putter_get_theme_option( 'related_slider', 0 ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = putter_get_theme_option( 'related_position', 'below_content' );
			$related_style = putter_get_theme_option( 'related_style', 'classic' );
			...
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				...
				if ( $args['slider'] ) {
					...
					$slider_args['slides_space']      = (int)putter_get_theme_option( 'related_slider_space', 30 );
					$slider_args['slider_controls']   = putter_get_theme_option( 'related_slider_controls', 'none' );
					$slider_args['slider_pagination'] = putter_get_theme_option( 'related_slider_pagination', 'bottom' );
...
if ( ! function_exists( 'putter_show_posts' ) ) {
	...
	function putter_show_posts( $args = array() ) {
		...
		$class = apply_filters( 'putter_filter_posts_container_classes', 'posts_container'
				...
				. ( ! putter_is_off( $custom_style )
					...
					: ( putter_is_blog_style_use_masonry( $style )
						...
						: ( $columns > 1 || apply_filters( 'putter_filter_columns_wrap_for_single_column', false )
		...
		if ( $args['sticky'] ) {
			...
			if ( (int)putter_get_theme_option( 'first_post_large', 0 ) > 0 && ! is_paged() && ! in_array( putter_get_theme_option( 'body_style' ), array( 'fullwide', 'fullscreen' ) ) && have_posts() ) {
				the_post();
