...
if ( ! function_exists( 'putter_array_str_replace' ) ) {
	/**
	 ...
	 */
	function putter_array_str_replace( $from, $to, $key, $arr ) {
		foreach ( $arr as $k => $v ) {
			if ( is_array( $v ) ) {
				$arr[ $k ] = putter_array_str_replace( $from, $to, $key, $v );
			} else if ( $k == $key ) {
...
if ( ! function_exists( 'putter_get_svg_from_file' ) ) {
	...
	function putter_get_svg_from_file( $svg ) {
		if ( putter_is_url( $svg ) && ! putter_is_external_url( $svg ) ) {
			$svg_path = putter_url_to_local_path( $svg );
			if ( ! empty( $svg_path ) ) {
				$svg = $svg_path;
			}
		}
			...
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							...
							apply_filters( 'putter_filter_cookie_options', array(
								...
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//putter_get_domain_from_url( home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//putter_get_domain_from_url( home_url() ),
							$secure,
							$httponly
						);
			}
...
if ( ! function_exists( 'putter_rgba2hex' ) ) {
	/**
	 ...
	 */
	function putter_rgba2hex( $color, $alpha = false ) {
...
if ( ! function_exists( 'putter_is_external_url' ) ) {
	...
	function putter_is_external_url( $url ) {
		...
		return putter_is_url( $url ) && strpos( $url, putter_remove_protocol_from_url( $parts[0], true ) ) === false;
	}
}
