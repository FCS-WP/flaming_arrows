...
$putter_theme_slug = get_template();
...
<div class="putter_admin_notice putter_rate_notice notice notice-info is-dismissible" data-notice="rate">
	...
	// Title
	$putter_theme_name = '"' . $putter_theme_obj->get( 'Name' ) . ( PUTTER_THEME_FREE ? ' ' . __( 'Free', 'putter' ) : '' ) . '"';
	?>
	<h3 class="putter_notice_title"><a href="<?php echo esc_url( putter_storage_get( 'theme_rate_url' ) ); ?>" target="_blank">
		...
		echo esc_html(
			sprintf(
				...
				__( 'Help Us Grow - Rate %s Today!', 'putter' ),
				$putter_theme_name
			)
		);
		?>
	...
	<div class="putter_notice_text">
		<p><?php
			// Translators: Add theme name to the 'Welcome' message
			echo wp_kses_data( sprintf( __( "Thank you for choosing the %s theme for your website! We're excited to see how you've customized your site, and we hope you've enjoyed working with our theme.", 'putter' ), $putter_theme_name ) );
		?></p>
		<p><?php
			// Translators: Add theme name to the 'Welcome' message
			echo wp_kses_data( sprintf( __( "Your feedback really matters to us! If you've had a positive experience, we'd love for you to take a moment to rate %s and share your thoughts on the customer service you received.", 'putter' ), $putter_theme_name ) );
		?></p>
	</div>
	<?php
	...
	<div class="putter_notice_buttons">
		...
		<a href="<?php echo esc_url( putter_storage_get( 'theme_rate_url' ) ); ?>" class="button button-primary" target="_blank"><i class="dashicons dashicons-star-filled"></i> 
			...
			echo esc_html( sprintf( __( 'Rate %s Now', 'putter' ), $putter_theme_name ) );
			?>
		</a>
		<?php
