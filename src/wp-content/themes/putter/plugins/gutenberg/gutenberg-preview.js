...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			putter_create_observer( 'putter-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				putter_gutenberg_editor_init();
			} );
		} else {
		 	putter_gutenberg_editor_init();
		}
	...
	function putter_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'putter_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'putter_inited' ) ) {
	...
	// Init on page load
	function putter_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = putter_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + PUTTER_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + PUTTER_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in PUTTER_STORAGE['override_classes'] ) {
				$obj.addClass( PUTTER_STORAGE['override_classes'][i].replace( '%s', PUTTER_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( putter_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && PUTTER_STORAGE['sidebar_position'] != 'hide' ) {
				if ( putter_apply_filters( 'putter_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'putter_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( putter_apply_filters( 'putter_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				putter_remove_observer( 'putter-check-editor-styles-wrapper' );
				putter_create_observer( 'putter-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'putter_inited' );
	}
} );
