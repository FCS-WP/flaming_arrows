...
jQuery( document ).ready(
	function() {
		...
		putter_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="putter_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.putter_post_editor' );
						if ( link.length > 0 ) {
							putter_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.putter_post_editor', function(e) {
						putter_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('putter_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function putter_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
