...
if ( ! function_exists( 'putter_theme_setup9' ) ) {
	...
	function putter_theme_setup9() {
		...
		register_nav_menus( apply_filters( 'putter_filter_register_nav_menus', array(
...
if ( ! function_exists( 'putter_wp_styles' ) ) {
	...
	function putter_wp_styles() {
		...
		if ( is_front_page() && ! empty( $bg_image ) && putter_is_on( putter_get_theme_option( 'front_page_enabled', false ) ) ) {
			...
			putter_add_inline_css( 'body.frontpage, body.home-page, body.home { background-image:url(' . esc_url( $bg_image ) . '); }' );
		} else {
			...
			if ( ! empty( $bg_image ) && ( putter_get_theme_option( 'body_style' ) == 'boxed' || is_customize_preview() ) ) {
				putter_add_inline_css( 'body.body_style_boxed { background-image:url(' . esc_url( putter_clear_thumb_size( $bg_image ) ) . '); }' );
			}
		}
...
if ( ! function_exists( 'putter_wp_styles_single' ) ) {
	...
	function putter_wp_styles_single() {
		if ( apply_filters( 'putter_filters_separate_single_styles', false )
			&& apply_filters( 'putter_filters_load_single_styles',
								putter_is_single()
								|| putter_is_singular( 'attachment' )
								|| (int)putter_get_theme_option( 'open_full_post_in_blog', 0 ) > 0
								|| ( putter_is_singular( 'page' ) && ! is_front_page() && ( comments_open() || get_comments_number() ) )
							)
		) {
...
if ( ! function_exists( 'putter_wp_styles_single_responsive' ) ) {
	...
	function putter_wp_styles_single_responsive() {
		if ( apply_filters( 'putter_filters_separate_single_styles', false )
			&& apply_filters( 'putter_filters_load_single_styles', putter_is_single() || putter_is_singular( 'attachment' ) || (int)putter_get_theme_option( 'open_full_post_in_blog', 0 ) > 0 )
		) {
			if ( putter_is_off( putter_get_theme_option( 'debug_mode' ) ) ) {
...
if ( ! function_exists( 'putter_wp_scripts' ) ) {
	...
	function putter_wp_scripts() {
		...
		if ( (int)putter_get_theme_option( 'single_parallax', 0 ) != 0
			&& ( putter_is_singular( 'post' )
				|| ( (int)putter_get_theme_option( 'open_full_post_in_blog', 0 ) > 0 && ( is_home() || is_archive() || is_category() ) )
				)
		) {
		...
		if ( $blog_archive && ! is_customize_preview() && ! putter_is_off( putter_get_theme_option( 'show_filters', 0 ) ) ) {
...
if ( ! function_exists( 'putter_localize_scripts' ) ) {
	...
	function putter_localize_scripts() {
		...
		wp_localize_script( 'putter-init', 'PUTTER_STORAGE', apply_filters( 'putter_filter_localize_script', array(
			...
			// Sidemenu options
			'menu_side_stretch'   => (int) putter_get_theme_option( 'menu_side_stretch', 0 ) > 0,
			'menu_side_icons'     => (int) putter_get_theme_option( 'menu_side_icons', 0 ) > 0,
			...
			'open_full_post'      => (int)putter_get_theme_option( 'open_full_post_in_blog', 0 ) > 0,

			// Which block to load in the single posts
			'which_block_load'    => putter_get_theme_option( 'posts_navigation_scroll_which_block', 'article' ),
...
if ( ! function_exists( 'putter_add_body_classes' ) ) {
	...
	function putter_add_body_classes( $classes ) {
		...
		if ( putter_sidebar_present() ) {
			...
			$classes[] = 'sidebar_small_screen_' . esc_attr( putter_get_theme_option( 'sidebar_position_ss', 'below' ) );
		} else {
		...
		if ( putter_get_theme_option( 'blog_content', 'excerpt' ) == 'fullpost' ) {
...
if ( ! function_exists( 'putter_wp_footer' ) ) {
	...
	function putter_wp_footer() {
		// Add header zoom
		$header_zoom = max( 0.2, min( 2, (float)putter_get_theme_option( 'header_zoom', 1 ) ) );
		if ( 1 != $header_zoom ) {
			putter_add_inline_css( ".sc_layouts_title_title{font-size:{$header_zoom}em}" );
		}
		// Add logo zoom
		$logo_zoom = max( 0.2, min( 2, (float)putter_get_theme_option( 'logo_zoom', 1 ) ) );
		if ( 1 != $logo_zoom ) {
...
if ( defined( 'PUTTER_ALLOW_SKINS' ) && PUTTER_ALLOW_SKINS ) {
	// Load the permanent file with themes-group functions (not overridable after the engine updated)
	if ( file_exists( PUTTER_THEME_DIR . 'skins/skins-setup.php' ) ) {
		require_once PUTTER_THEME_DIR . 'skins/skins-setup.php';
	}
	if ( file_exists( PUTTER_THEME_DIR . 'skins/skins.php' ) ) {
...
require_once putter_get_file_dir( 'theme-specific/theme-improves.php' );
...
if ( ! function_exists( 'putter_load_required_plugins' ) ) {
	add_action( 'after_setup_theme', 'putter_load_required_plugins', -1 );
	/**
	...
	function putter_load_required_plugins() {
