...
jQuery( document ).ready( function() {
				...
				if ( data ) {
					...
					for ( i in data ) {
						...
						if ( param.length == 2 && param[0].slice(-6) != '_nonce' ) {
							...
							} else if ( fld.hasClass('putter_color_selector') ) {
								// Update global color selector
								if ( val.slice( 0, 4 ) == 'var(' ) {
									globals = fld.parents('.putter_options_item_field').find('.putter_color_selector_globals');
									if ( globals.length > 0 ) {
										globals.find( '.putter_color_selector_globals_list_item_active' ).removeClass( 'putter_color_selector_globals_list_item_active' );
										item_globals = globals.find( '.putter_color_selector_globals_list_item[data-value="' + val + '"]' );
										...
										setTimeout( function() {
											...
											item_globals.addClass( 'putter_color_selector_globals_list_item_active' );
							...
							} else if ( fld.next().hasClass('putter_icon_selector') ) {
								if ( val == '' || val == 'none' ) {
									fld.next().attr( 'class', 'putter_icon_selector' );
								} else {
	...
	function putter_options_init_fields(e, container) {
			...
			jQuery( this )
				...
				.on( 'click', '.putter_options_clone > .putter_options_clone_control_add', function ( e ) {
					var clone_obj = jQuery(this).parents('.putter_options_clone'),
						group = clone_obj.parents('.putter_options_group');
					// Clone fields
					putter_options_clone(clone_obj, true);
					// Enable/Disable clone buttons
					putter_options_clone_toggle_buttons(group);
					// Mark group as changed
					group.data( 'param-changed', 1 ).find('[data-param]').data( 'param-changed', 1 );
				...
				jQuery( this )
					.sortable( {
						...
						update: function( event, ui ) {
							...
							// Mark group as changed
							ui.item.parents('.putter_options_group').data( 'param-changed', 1 ).find('[data-param]').data( 'param-changed', 1 );
						}
					});
			}
		});
		...
		function putter_options_clone( obj, copy_values ) {
			...
			clone.find('.putter_options_item_field :input').each( function() {
				...
				if (input.attr('name') && input.attr('name').indexOf("putter_options_field_") === 0) {
					input.on( 'change', function () {
						jQuery( this ).parents('.putter_options_group').data( 'param-changed', 1 ).find('[data-param]').data( 'param-changed', 1 );
					} );
				}
			});
			//Remove UI sliders
	...
	jQuery( '#putter_options_tabs' )
		.on( 'keydown', '.putter_options_responsive_button', function( e ) {
			// If 'Enter' or 'Space' is pressed - trigger click on this object
			if ( [ 13, 32 ].indexOf( e.which ) >= 0 ) {
				jQuery( this ).trigger( 'click' );
				e.preventDefault();
		...
		.on( 'click', '.putter_options_responsive_button', function (e) {
			var $wrap = jQuery( '.putter_options' ),
				$buttons_wrap = jQuery( this ).parents( '.putter_options_responsive_buttons' ),
				cur_mode = $wrap.data( 'responsive' ),
				new_mode = jQuery( this ).data( 'responsive' );
			if ( ! $buttons_wrap.hasClass( 'opened' ) ) {
				jQuery( '.putter_options_responsive_buttons' ).removeClass( 'opened' );
